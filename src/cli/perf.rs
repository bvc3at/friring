...
//! perf timing is active — `FRIRING_PERF_LOG=1` or an open perf HUD
//! (`<leader> m`, or `F12` when `[prefix] mode = "off"`).
...
const NO_SNAPSHOT_HINT: &str = "No perf snapshot published. Run the TUI with FRIRING_PERF_LOG=1 \
     or open its perf HUD (<leader> m, or F12 with the leader off), then retry.";
