...
pub(crate) fn render(
    ...
    frame.render_widget(
        Paragraph::new(Line::from(Span::styled(
            " j/k scroll · w wrap · Enter fold · h tree · Esc close",
            dim(),
        ))),
        footer,
    );
    ...
    while i < state.rows.len() && y < bottom {
        let lines = row_visual_lines(state, i, width, show_cursor);
        let start_y = y;
        for line in &lines {
            if y >= bottom {
                break;
...
fn row_height(state: &CcActivityState, i: usize, width: usize) -> usize {
    row_visual_lines(state, i, width, false).len()
}

/// The visual lines for one logical transcript row (wrap / h-scroll applied to
/// bodies). The selected row's lines are reverse-highlighted when `cursor`.
fn row_visual_lines(
    ...
    let mut lines = match &state.rows[i] {
        ...
        CcRow::Text(s) => body_lines(s, normal(), state.wrap, state.h_scroll, width),
        CcRow::Block(bi) => block_lines(state, *bi, width),
    };
    if lines.is_empty() {
...
fn block_lines(state: &CcActivityState, bi: usize, width: usize) -> Vec<Line<'static>> {
    ...
    match block {
        TranscriptBlock::Prompt(s) => {
            ...
            out.extend(body_lines(s, dim(), state.wrap, state.h_scroll, width));
            out
        }
        ...
        TranscriptBlock::Text(s) => {
            let mut out = body_lines(s, normal(), state.wrap, state.h_scroll, width);
        ...
        TranscriptBlock::ToolUse { name, input } => {
            ...
            if collapsed {
                ...
                out.extend(body_lines(input, dim(), state.wrap, state.h_scroll, width));
                out
            }
        }
