...
| `ghost_scrollback_lines` | `1000` | scrollback lines captured into a ghost's frozen frame at unload/shutdown (`0` = visible screen only, ~5 KB; each 1000 lines ≈ 50–100 KB per session in the DB). Clamped to 10 000 by the capture path, and friring's tmux sessions run with `history-limit = 5000`, so above ~5 000 there is no further history to capture |
